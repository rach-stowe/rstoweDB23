...
SELECT climb_name AS 'Name', 
       grade_str AS 'Grade',
       COALESCE(climb_len_ft, '') AS 'Length (ft)',
       crag_name AS 'Crag',
       COALESCE(first_ascent, '') AS 'First ascent by',
       COALESCE(GROUP_CONCAT(DISTINCT CONCAT(climber_first_name, ' ', climber_last_name)), '') AS 'Equipped by'
  FROM first_climbs
